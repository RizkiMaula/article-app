...
export default function Login() {
  ...
  const onSubmit = async (data) => {
    try {
      const url = isLogin ? 'https://test-fe.mysellerpintar.com/api/auth/login' : 'https://test-fe.mysellerpintar.com/api/auth/register';
      ...
      // Redirect atau lakukan sesuatu setelah login/register

      alert(isLogin ? 'Login berhasil!' : 'Registrasi berhasil!');
    } catch (error) {
      setErrorMessage(error.response?.data?.message || (isLogin ? 'Login gagal' : 'Registrasi gagal'));
    }
  };

  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8 border-2 border-black">
        <div>
          <Logo />
        </div>

        <form
          className="mt-8 space-y-6 border-2 flex flex-col gap-2 border-black"
          onSubmit={handleSubmit(onSubmit)}
        >
          {errorMessage && <div className="text-red-500 text-sm text-center">{errorMessage}</div>}

          <div className="rounded-md shadow-sm -space-y-px border-2 border-red-700 flex flex-col gap-3">
            {/* username */}
            ...
            {!isLogin && (
              ...
              //   </div>
              <div className="grid w-full border-2 border-amber-950 items-center gap-2">
                <Label htmlFor="Role">Role</Label>
                <Select className="w-[180px] border-2 border-amber-950">
        ...
        <div className="text-center">
          ...
          >
            {isLogin ? 'Belum punya akun? Daftar sekarang' : 'Sudah punya akun? Masuk sekarang'}
          </button>
        </div>
      </div>
    </div>
